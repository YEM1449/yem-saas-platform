...
| 3 | Module Commercial – version MVP | **DONE** | Contacts/Prospects + Deposits/Reservations + Sales Contracts + Commercial Dashboard + Receivables Dashboard + Commission Tracking + Discount Analytics + Prospect Source Funnel fully implemented through Phase 3. RBAC, tenant isolation, audit trail all covered. Commission rule CRUD with project-specific priority. Receivables KPIs (outstanding, overdue, aging buckets, collection rate). Discount analytics (avg/max discount %, top-10 agents by discount). Prospect source funnel (source grouping + conversion rates). Phase 4 adds: Client-facing buyer portal (`GET /api/portal/**`, ROLE_PORTAL, magic link auth) — buyers can self-service view contracts, payment schedules, property details. IT: `CommissionIT`, `ReceivablesDashboardIT`, `CommercialDashboardIT` (7 tests), `PortalAuthIT`, `PortalContractsIT`, `PortalPaymentsIT` (13 tests). |
