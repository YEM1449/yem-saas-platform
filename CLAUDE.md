...
- **P2 ✅** `VisiteService`: create/update/confirmer/marquerNoShow/annuler/enregistrerCompteRendu/lierVente + agenda/listByContact/countRealisees/countOpportunites. RG-V04 (AGENT→own, MANAGER/ADMIN→société via `restrictedToOwnAgent()` on `ROLE_AGENT`), RG-V05 conflict (`verifierConflit`, MANAGER override traced), RG-V06 compte-rendu-required, RG-V07 `planifierRappels` (future-dated H24 agent+prospect / H1 agent), RG-V08 cancel reminders. Exceptions `VisiteNotFoundException`(404)/`InvalidVisiteTransitionException`(409)/`ConflitVisiteException`(409)/`CompteRenduRequisException`(422) wired in `GlobalExceptionHandler` + `ErrorCode` (VISITE_NOT_FOUND/VISITE_CONFLIT/COMPTE_RENDU_REQUIS). DTOs Create/Update/CompteRendu/Annuler/VisiteResponse. 5 unit tests pass.
- **P3 ✅** `VisiteController` `/api/visites` (GET agenda ?agentId&statut&from&to, GET /{id}, POST, PUT, /confirmer, /no-show, /annuler, /compte-rendu; `@PreAuthorize` ADMIN/MANAGER/AGENT). **Persistent reminders** (SRE): `RappelVisiteJob` `@Scheduled(fixedDelay 5min)` + `runAsSystem` → `VisiteRappelService.envoyerRappelsDus()` scans `visite_rappel` (nil-UUID RLS bypass), sends via `VisiteEmailService` (Brevo `EmailSender`, FR templates, Casablanca rendering), idempotent ENVOYE, retry capped at MAX_TENTATIVES=5 then ANNULE. RG-V08 cancellation email wired into `VisiteService.annuler` (only when was CONFIRMEE). Config `app.visite.rappel-scan-ms`. 3 unit tests (`VisiteRappelServiceTest`) + 8 IT (`VisiteControllerIT`, Testcontainers — auth/create/conflict409/compte-rendu/annuler/cross-société404/reminder-job→ENVOYE) all green.
- **P4 ✅** Frontend `modules/visites/` (Angular 19 standalone, `@if`/`@for`, signals). Lazy route `/app/visites` + shell nav link. Services: `VisiteApiService` (mirrors backend DTOs), `VisiteStateService` (signals agenda state; window computed per view), `casablanca-time.ts` (RG-V10 wall-clock↔Instant + FR rendering). `AgendaComponent` (jour/semaine/mois, agent filter MANAGER/ADMIN, statut filter, statut colours, click empty slot→form, legend, intentional empty states). `VisiteFormComponent` (quick create + edit, reuses `app-contact-picker`/`app-user-picker`, cascading programme→bien, 409 conflict + manager override). `VisiteDetailComponent` (statut-gated actions confirmer/CR/no-show/annuler/modifier/.ics). `VisiteCompteRenduComponent` (RG-V06, OPPORTUNITE_CREEE→"Créer une vente" CTA). i18n `visites.*` (fr) + `nav.visites` (fr/en/ar). Build green.
- **P5 ✅** Intégrations. Backend: `VisiteController` expose `GET /by-contact/{contactId}`, `POST /{id}/lier-vente`, `GET /{id}/ics` (`VisiteService.genererIcs` — iCalendar UTC). `HomeDashboardDTO`+`HomeDashboardService` enrichis de `visitesRealiseesMoisCourant`+`tauxConversionVisites` via `VisiteService` injecté (RG-V09 source unique, mois Casablanca). `LierVenteRequest` DTO. Frontend: onglet **Visites** sur `contact-detail` (byContact + planifier pré-rempli), `vente-create` mode visite-origin (création contact/bien puis `lierVente`, RG-V06), carte KPI visites sur `home-dashboard`, `visite-form` prefill `contactId`, `visite-api` `byContact`/`lierVente`. Tests: 3 IT (by-contact, ics, lier-vente 400). Unit suite verte ; FE build verte.
- **P6 ✅** E2E `e2e/visites.spec.ts` (5 tests : agenda accessible, nav, bascule vues, ouverture formulaire, création→détail ; projet `visites-tests` dans `playwright.config.ts`) + data-testid sur agenda/formulaire. Docs: `docs/context/api-map.md` (endpoints Wave 16), `docs/business-rules/rg-referentiel.md` (RG-V01..RG-V10), `docs/guides/user/visites.md` (guide agent). **Wave 16 COMPLETE.**
