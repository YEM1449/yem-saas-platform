...
/**
 * Repository for Project entities with société-scoped queries and KPI aggregations.
...
public interface ProjectRepository extends JpaRepository<Project, UUID> {
    ...
    boolean existsBySocieteIdAndName(UUID societeId, String name);

    boolean existsBySocieteIdAndNameAndIdNot(UUID societeId, String name, UUID excludeId);
    ...
    @Query("SELECT p.status, COUNT(p) FROM com.yem.hlm.backend.property.domain.Property p " +
           "WHERE p.societeId = :societeId AND p.project.id = :projectId AND p.deletedAt IS NULL " +
           "GROUP BY p.status")
    List<Object[]> countPropertiesByStatus(@Param("societeId") UUID societeId, @Param("projectId") UUID projectId);
    ...
    @Query("SELECT p.type, COUNT(p) FROM com.yem.hlm.backend.property.domain.Property p " +
           "WHERE p.societeId = :societeId AND p.project.id = :projectId AND p.deletedAt IS NULL " +
           "GROUP BY p.type")
    List<Object[]> countPropertiesByType(@Param("societeId") UUID societeId, @Param("projectId") UUID projectId);
    ...
    @Query("SELECT COUNT(p) FROM com.yem.hlm.backend.property.domain.Property p " +
           "WHERE p.societeId = :societeId AND p.project.id = :projectId AND p.deletedAt IS NULL")
    long countTotalProperties(@Param("societeId") UUID societeId, @Param("projectId") UUID projectId);
    ...
    List<Object[]> depositStats(@Param("societeId") UUID societeId, @Param("projectId") UUID projectId);
