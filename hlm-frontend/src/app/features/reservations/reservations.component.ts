...
export class ReservationsComponent implements OnInit {
  ...
  private resolveNames(reservations: Reservation[]): void {
    ...
    this.http.get<Array<{ id: string; title: string; referenceCode: string }>>(
      ...
      next: ps => ps.forEach(p => this.propertyNames.set(p.id, `${p.title} · ${p.referenceCode}`)),
      error: () => {},
    });

    // Suppress unused variable warnings — used only for deduplication above
    void contactIds; void propertyIds;
  }
